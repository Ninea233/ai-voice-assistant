 ...
 * 设计上支持多引擎模型表决：
 *
 * ⚠️ 当前状态：**仅接入主引擎（mandarin）**。方言引擎的代码路径存在，
 *    但配置项 [asr] dialect_enabled 在 C++ 侧从未被读取，SetDialect() 无调用者；
 *    voting_strategy 亦只写不读（majority 未实现）。即现状恒为单引擎。
 * - 主引擎：讯飞中文识别大模型（WebSocket）
 * - 方言引擎：讯飞方言识别大模型（可启用/禁用）
 * - 表决策略：置信度加权（高置信度引擎优先采纳，相同文本累加置信度）
 *
